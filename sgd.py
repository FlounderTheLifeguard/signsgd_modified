...
class Parameter_server:
    ...
    def broadcast(self, theta0, theta_li, alpha, l1_lambda, weight_lambda, delta):
        ...
        for i, mac in enumerate(self.machines):
            ...
            for j in range(num_local_iter):
                new_theta = mac.update(theta0, new_theta, alpha, l1_lambda, weight_lambda, delta)
            new_theta_li.append(new_theta)
        tmp = np.zeros_like(theta0)
        machine_sampled = random.sample(range(num_machines), num_sampled_machine)
        for i in machine_sampled:
            # L1 norm
    ...
    def plot_curve(self):
        ...
        plt.title("sgd,non-regulization,sync")
        plt.show()

        sss = 'sgd,non-regularization,sync,5,调参,sampled12'
        f = open('D:/OneDrive/桌面/实验/code/RSA-Byzantine-master/RSA-code/result2/noniid-5/' + sss, 'wb')
