...
num_byz = 0  # number of Byzantine workers
...
class Machine:
    ...
    def update2(self, theta0, theta0_last, theta, theta_last, alpha, l1_lambda, l1_gamma, weight_lambda, delta):
        """Calculates gradient with a randomly selected sample, given the current theta
         Accepts theta, a np array with shape of (dimension,)
         Returns the calculated gradient"""
        m = self.data_x.shape[0]
        ...
        if Sync == 'sync':
            grad_f = cal_total_grad(self.data_x[id:(id + batch_size)], self.data_y[id:(id + batch_size)], theta0,
                                    weight_lambda)
            g_norm = l1_lambda * np.sign(theta0 - theta0_last)
            g_norm_worker = l1_gamma * np.sign(theta - theta_last)
        if Sync == 'async':
            grad_f = cal_total_grad(self.data_x[id:(id + batch_size)], self.data_y[id:(id + batch_size)], theta,
                                    weight_lambda)
            g_norm = l1_lambda * np.sign(theta - theta0)
        ...
        # grad = grad_f / num_machines + l1_lambda * np.sign(theta - theta0)
        if Rgl == 'regularization':
            grad = grad_f + g_norm - g_norm_worker # + l1_lambda * np.sign(theta - theta0)
        if Rgl == 'non-regularization':
            grad = grad_f
        # 改为从theta0更新
        if Sync == 'sync':
...
class Parameter_server:
    ...
    def broadcast(self, theta0, theta0_last, theta_li, theta_li_last, alpha, l1_lambda, l1_gamma, weight_lambda, delta):
    #def broadcast(self, theta0, theta0_last, theta_li, theta_li_last , alpha, l1_lambda, weight_lambda, delta):
    #def broadcast(self, theta0, theta_li, alpha, l1_lambda, weight_lambda, delta):
        ...
        for i, mac in enumerate(self.machines):
            ...
            new_theta = theta_li[i]
            theta_last = theta_li_last[i]

            for j in range(num_local_iter):
                #print(j)

                #new_theta_li.append(mac.update(theta0, theta_li[i], alpha, l1_lambda, weight_lambda, delta))
                #new_theta = mac.update(theta0, theta0_last, new_theta , alpha, l1_lambda, weight_lambda, delta)
                new_theta = mac.update2(theta0, theta0_last, new_theta, theta_last, alpha, l1_lambda, l1_gamma, weight_lambda, delta)

                #print(type(new_theta))

                #new_theta_li.append(mac.update(theta0, theta0_last, theta_li[i], alpha, l1_lambda, weight_lambda, delta))
            new_theta_li.append(new_theta)
        ...
        for i in range(len(theta_li)):
            ...
            if exit_byzantine == False:
                if Aggr == 'signsgd' or 'signsgdforreal':
        ...
        #print(tmp)
        # if Aggr == 'signsgdforreal':
        #     tmp = np.sign(tmp)
        #     new_theta0 = theta0 - alpha * (tmp + weight_lambda * theta0) * alpha2
        if Aggr == 'signsgd'or 'signsgdforreal':
            new_theta0 = theta0 - alpha * (tmp + weight_lambda * theta0) * alpha2
        if Aggr == 'sgd':
            tmp = tmp / 20
            new_theta0 = theta0 - (tmp + weight_lambda * theta0 * alpha) * alpha2
            #print(alpha)
        #print(tmp)
    ...
    def train(self, init_theta0, init_theta, alpha, l1_lambda, l1_gamma, weight_lambda):
        ...
        for i in range(num_iter):
            alpha = d / np.sqrt(i + 1) * alpha1

            rec_theta0, rec_theta = self.broadcast(self.theta0_li[-1], self.theta0_li[-2], self.theta_li[-1],self.theta_li[-2], alpha, l1_lambda,
                                                   l1_gamma, weight_lambda, delta)
            #rec_theta0, rec_theta = self.broadcast(self.theta0_li[-1], self.theta0_li[-2],self.theta_li[-1] , alpha, l1_lambda, weight_lambda, delta)
            ...
            if (i + 1) % 10 == 0:
                ...
                if (Rgl == 'regularization') & (Sync == 'sync'):
                    loss = cal_loss(self.test_img_bias, self.test_lbl, rec_theta0, weight_lambda)
                           #+ l1_lambda * np.sum(np.maximum(self.theta0_li[-1]-self.theta0_li[-2], self.theta0_li[-2]-self.theta0_li[-1]))
    ...
    def plot_curve(self):
        ...
        sss = Aggr + "," + Rgl + "," + Sync + "," + str(num_local_iter) + "," +"调参"
        f = open('D:/OneDrive/桌面/实验/code/RSA-Byzantine-master/RSA-code/result2/noniid-2-regularization/' + sss ,'wb')
        ...
        plt.savefig('D:/OneDrive/桌面/实验/code/RSA-Byzantine-master/RSA-code/result2/noniid-2-regularization/' + sss + ',acc.png')
        ...
        plt.ylabel('loss')
        plt.title(Aggr + "," + Rgl + "," + Sync + "," + str(num_local_iter))
        plt.savefig('D:/OneDrive/桌面/实验/code/RSA-Byzantine-master/RSA-code/result2/noniid-2-regularization/' + sss + ',loss.png')
...
def main():
    ...
    l1_gamma = l1_gamma1
    weight_lambda = 0.01
    server.train(init_theta0, init_theta, alpha, l1_lambda, l1_gamma, weight_lambda)
    server.plot_curve()
